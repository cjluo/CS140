...
inode_extend (struct inode *inode, off_t offset)
{
  ...
  block_sector_t sectors = offset / BLOCK_SECTOR_SIZE;
  // printf ("## inode_extend %d\n", (int)sectors);
  sectors -= DIRECT_SIZE;
  ...
  // if ((int)disk_inode->sector_direct[sectors] != -1)
  // {
